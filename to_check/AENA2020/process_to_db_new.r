...
createMissingTables <- function(config) {
    ...
    sapply(dataSetIds, function(ds) {
      tryCatch({
        if(any(grepl(x,firstFullTable))) {
          sqlSave(conn, createEmptyDfForLevel(paste(ds,'_',x,'_001', sep = ''), conn, tableNames, firstFullTable), tablename = paste(ds,'_',x,'_001', sep = ''), rownames = F)
          MissingTables <- MissingTables[MissingTables != paste(ds,'_',x,'_001', sep = '')]
        } else {
          sqlSave(conn, fakeDataframe, tablename = paste(ds,'_',x,'_001', sep = ''))
          MissingTables <- MissingTables[MissingTables != paste(ds,'_',x,'_001', sep = '')]
        }
        #}
      }, error = function(e){
        #print(e)
        message(paste("Table ",ds,'_',x,'_001 cannot be written:',e, sep = ''))
      })
    })
  })
  ...
  #   if(!(any(grepl(unlist(strsplit(i,"_"))[2],firstFullTable)))) {
  #     next
  #   }
  #   tryCatch({
  #     sqlSave(conn,createEmptyDfForLevel(i, conn, tableNames, firstFullTable),tablename = i,rownames = F)
  #     setPrimaryKeys(unlist(strsplit(i,"_"))[2], i, geoLevelInformation, conn, "character")
  #   }, error = function(e) {
